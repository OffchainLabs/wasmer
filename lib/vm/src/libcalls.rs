...
pub unsafe extern "C" fn wasmer_vm_memory32_copy(
    ...
    // This function is at an FFI boundary: the caller of this function is
    // generated by wasmer's LLVM compiler, while this function itself is
    // compiled via a Rust compiler.
    //
    // We have observed that LLVM will omit the `trunc` operation from i64 to i32.
    // Our best guess is that LLVM thinks the callee function will simply use
    // the parameters as u32, ignoring the upper 32 bits.
    //
    // To the contrary, the compiled version of this function (from rustc) will
    // assume that the upper 32 bits of the parameters will be full zeros.
    // This will trigger a runtime bug. As this function is really used across
    // FFI boundaries, it is hard to deduce if either side has a bug.
    //
    // As a result, we will have to tweak the signature to accept full 64 bits
    // values, and cast them to 32 bits by ourselves. In our tests, this change
    // ensures LLVM generated caller works with rustc compiled callee function.
    let dst = dst as u32;
    let src = src as u32;
    let len = len as u32;
